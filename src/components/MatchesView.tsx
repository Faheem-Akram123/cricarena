          ...
          <div className="bg-white rounded-3xl p-6 w-full max-w-lg shadow-2xl relative border border-slate-100 flex flex-col max-h-[92vh]">
            ...
            {modalType === 'lineups' && selectedMatch && (
              <div className="space-y-4 overflow-y-auto pr-1">
                ...
                <p className="text-[11px] text-slate-400 font-semibold mb-1">Check players to nominate for this official match squadron.</p>
                ...
                <div className="grid grid-cols-2 gap-4 max-h-[300px] overflow-y-auto pr-1 border border-slate-100 rounded-2xl p-3">
                  ...
                  <div className="space-y-2 border-r border-slate-100 pr-2">
                    ...
                    <div className="space-y-1">
                      {players.filter(p => p.team === selectedMatch.team1).map(p => {
                        const inList = t1SquadList.includes(p.id);
                        return (
                          <label key={p.id} className="flex items-center gap-2 text-[11px] font-bold text-slate-600 cursor-pointer p-1.5 rounded-lg hover:bg-slate-50 select-none">
                            <input 
                              type="checkbox" 
                              checked={inList} 
                              onChange={() => {
                                if (inList) setT1SquadList(prev => prev.filter(x => x !== p.id));
                                else setT1SquadList(prev => [...prev, p.id]);
                              }}
                  ...
                  <div className="space-y-2 pl-1">
                    ...
                    <div className="space-y-1">
                      {players.filter(p => p.team === selectedMatch.team2).map(p => {
                          ...
                          <label key={p.id} className="flex items-center gap-2 text-[11px] font-bold text-slate-600 cursor-pointer p-1.5 rounded-lg hover:bg-slate-50 select-none">
                            <input 
                              ...
                              onChange={() => {
                                ...
                                else setT2SquadList(prev => [...prev, p.id]);
                              }}
                              className="accent-[#0B9B4D] rounded w-3.5 h-3.5" 
                            />
                            {p.name}
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 pt-3 border-t border-slate-100">
                  <div className="text-[10px] text-slate-550 font-bold bg-slate-50 border border-slate-205 px-3 py-1.5 rounded-lg">
                    Selected: {getTeam(selectedMatch.team1).emoji} {t1SquadList.length} vs {getTeam(selectedMatch.team2).emoji} {t2SquadList.length}
                  </div>
